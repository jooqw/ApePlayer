...
bool HDParser::load(const std::string& filename) {
    ...
    if (size < 0x10 || std::memcmp(data.data() + 0x0C, "SShd", 4) != 0) return false;
...
void HDParser::parse() {
    ...
    if (prog_offset != 0xFFFFFFFF && prog_offset < data.size()) parse_programs(prog_offset);
    if (breath_offset != 0xFFFFFFFF && breath_offset < data.size()) parse_breath_waves(breath_offset);
}
...
void HDParser::parse_programs(u32 base_offset) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        // Determine tone count
        int tone_count = (prog->type == 0xFF) ? data[abs_offset + 7] : (prog->type & 0x7F) + 1;
        u32 current_tone_offset = abs_offset + 8;

        for (int t = 0; t < tone_count; t++) {
            if (current_tone_offset + 16 > data.size()) break;
            Tone tone;
            tone.min_note = data[current_tone_offset + 0];
            tone.max_note = data[current_tone_offset + 1];
            tone.root_key = data[current_tone_offset + 2];
            tone.pitch_fine = (s8)data[current_tone_offset + 3];

            u16 bd_mult = Util::readU16(data, current_tone_offset + 4);
            tone.bd_offset = bd_mult * 8;

            tone.adsr1 = Util::readU16(data, current_tone_offset + 6);
            u16 raw_adsr2 = Util::readU16(data, current_tone_offset + 8);

            u8 vol_override = data[current_tone_offset + 10];
            tone.vol = data[current_tone_offset + 11];
            tone.pan = data[current_tone_offset + 12];
            tone.pitch_mult = data[current_tone_offset + 13];
            tone.breath_idx = data[current_tone_offset + 14];
            tone.flags = data[current_tone_offset + 15];

            // FIXME not sure??
            tone.adsr2 = raw_adsr2 ^ (u16)vol_override;
...
void HDParser::parse_breath_waves(u32 base_offset) {
    u16 bound = Util::readU16(data, base_offset);
    int count = bound + 1;
    u32 ptr_table = base_offset + 2;
    ...
    for (int i = 0; i < count; i++) {
        if (ptr_table + (i * 2) + 2 > data.size()) break;
        u16 rel_offset = Util::readU16(data, ptr_table + (i * 2));
        offsets.push_back(base_offset + rel_offset);
    }
...
void HDParser::print_debug_info() const {
    std::cout << "HD Info: " << programs.size() << " Programs, " << breath_scripts.size() << " Breath Scripts." << std::endl;
}
...
DecodedSample EngineUtils::decode_adpcm(const std::vector<u8>& adpcm_data) {
    ...
    for (int b = 0; b < num_blocks; b++) {
        ...
        if (flags & 1) {
            ...
            result.loop_end = (int)samples.size() + 28;
        }
...
int16_t EngineUtils::ps2_vol_to_cb(u8 vol) {
    if (vol == 0) return 1440; // Silence in centibels
...
int16_t HardwareADSR::simulate_timecents(u32 reg_val, Phase target_phase) {
    ...
    if (sim.envelope.counter_increment == 0) return 8000; // Approx infinite

    int samples = 0;
    int limit = 44100 * 15; // 15 second limit

    while (samples < limit) {
        bool running = sim.envelope.Tick(sim.current_volume);

        bool finished = false;
        if (target_phase == Phase::Attack) finished = (sim.current_volume >= 32767);
        else finished = (sim.current_volume <= 0);

        if (finished) break;
        samples++;
    }

    if (samples <= 1) return -32768; // Instant
...
// Reverb values are stored inside the executable and switched like presets
// here i just used the studio large preset.
// =============================================================
...
void ReverbEngine::process(const std::vector<float>& in_l, const std::vector<float>& in_r, std::vector<float>& out_l, std::vector<float>& out_r) {
    out_l.resize(in_l.size());
    out_r.resize(in_r.size());

    for (size_t i = 0; i < in_l.size(); i++) {
        s16 lin = Util::clamp16((int)(in_l[i] * 32767.0f));
        s16 rin = Util::clamp16((int)(in_r[i] * 32767.0f));

        s32 Lin = (lin * regs.vLIN) >> 15;
        s32 Rin = (rin * regs.vRIN) >> 15;
        ...
        l_same += mLSAME_2_val;
        ram[current_addr % (RAM_SIZE/2)] = Util::clamp16(l_same);

        auto write_ram = [&](u32 rel, s16 v) { ram[(current_addr + (rel&RAM_MASK)) % (RAM_SIZE/2)] = v; };

        // Redoing IIR with explicit writes
        write_ram(regs.mLSAME, Util::clamp16(l_same));
        ...
        r_same += mRSAME_2_val;
        write_ram(regs.mRSAME, Util::clamp16(r_same));
        ...
        l_diff += mLDIFF_2_val;
        write_ram(regs.mLDIFF, Util::clamp16(l_diff));
        ...
        r_diff += mRDIFF_2_val;
        write_ram(regs.mRDIFF, Util::clamp16(r_diff));

        // --- Comb Section ---
        s32 Lout = ((regs.vCOMB1 * rev_read(ram, current_addr, base_addr, regs.mLCOMB1)) +
        (regs.vCOMB2 * rev_read(ram, current_addr, base_addr, regs.mLCOMB2)) +
        (regs.vCOMB3 * rev_read(ram, current_addr, base_addr, regs.mLCOMB3)) +
        (regs.vCOMB4 * rev_read(ram, current_addr, base_addr, regs.mLCOMB4))) >> 15;

        s32 Rout = ((regs.vCOMB1 * rev_read(ram, current_addr, base_addr, regs.mRCOMB1)) +
        (regs.vCOMB2 * rev_read(ram, current_addr, base_addr, regs.mRCOMB2)) +
        (regs.vCOMB3 * rev_read(ram, current_addr, base_addr, regs.mRCOMB3)) +
        (regs.vCOMB4 * rev_read(ram, current_addr, base_addr, regs.mRCOMB4))) >> 15;

        // --- Allpass Section ---
        s16 mLAPF1_d_val = rev_read(ram, current_addr, base_addr, regs.mLAPF1 - regs.dAPF1);
        Lout = Lout - ((regs.vAPF1 * mLAPF1_d_val) >> 15);
        write_ram(regs.mLAPF1, Util::clamp16(Lout));
        Lout = ((Lout * regs.vAPF1) >> 15) + mLAPF1_d_val;

        s16 mRAPF1_d_val = rev_read(ram, current_addr, base_addr, regs.mRAPF1 - regs.dAPF1);
        Rout = Rout - ((regs.vAPF1 * mRAPF1_d_val) >> 15);
        write_ram(regs.mRAPF1, Util::clamp16(Rout));
        Rout = ((Rout * regs.vAPF1) >> 15) + mRAPF1_d_val;

        s16 mLAPF2_d_val = rev_read(ram, current_addr, base_addr, regs.mLAPF2 - regs.dAPF2);
        Lout = Lout - ((regs.vAPF2 * mLAPF2_d_val) >> 15);
        write_ram(regs.mLAPF2, Util::clamp16(Lout));
        Lout = ((Lout * regs.vAPF2) >> 15) + mLAPF2_d_val;

        s16 mRAPF2_d_val = rev_read(ram, current_addr, base_addr, regs.mRAPF2 - regs.dAPF2);
        Rout = Rout - ((regs.vAPF2 * mRAPF2_d_val) >> 15);
        write_ram(regs.mRAPF2, Util::clamp16(Rout));
        Rout = ((Rout * regs.vAPF2) >> 15) + mRAPF2_d_val;

        out_l[i] = (float)((Lout * regs.vLOUT) >> 15) / 32767.0f;
        out_r[i] = (float)((Rout * regs.vROUT) >> 15) / 32767.0f;

        current_addr = (current_addr + 1);
        if (current_addr > 0x3FFFE) current_addr = base_addr;
    }
}
...
float VibratoEngine::get_pitch_offset() const {
    ...
    double val0 = (double)lfo_table[idx0];
    double val1 = (double)lfo_table[idx1];
    double interp = val0 + (val1 - val0) * frac;

    double center_offset = (interp / 255.0) - 0.5;
...
void SPU::note_on(int ch_idx, int note, int vel) {
    ...
    const Tone* target_tone = nullptr;
    ...
    if (smp.pcm.empty() && !target_tone->is_noise()) return;
    ...
    v.data = smp;
    v.pos = 0.0;
    v.note_base_freq = base_pitch;
    v.base_pitch_mult = 1.0;
    v.target_pitch_mult = 1.0;
    v.noise_mode = target_tone->is_noise();
    ...
    if (ch.portamento_active && ch.last_note_pitch > 0.0) {
        ...
        v.sliding = false;
        v.portamento_step = 1.0;
    }
    ch.last_note_pitch = v.note_base_freq * v.target_pitch_mult;

    // Vibrato Setup
    bool will_have_vibrato = false;
    if (target_tone->use_modulation()) {
        int breath_idx = -1;
        if (target_tone->use_prog_breath()) breath_idx = prog->breath_idx;
        else breath_idx = target_tone->breath_idx;

        if (breath_idx != 0xFF && breath_idx != 0x7F && breath_idx < hd->breath_scripts.size()) {
            will_have_vibrato = true;
    ...
    v.note_key = note;
    v.active = true;
    v.reverb_on = target_tone->is_reverb();
    v.adsr = adsr;
...
void SPU::advance_vibrato_ticks(int ticks) {
    if (ticks <= 0) return;
    for (auto& v : active_voices) {
        if (v.vibrato_enabled && v.vibrato_attack_counter < v.vibrato_attack_time) {
            v.vibrato_attack_counter += (v.vibrato_attack_speed * ticks);
            if (v.vibrato_attack_counter > v.vibrato_attack_time) v.vibrato_attack_counter = v.vibrato_attack_time;
        }
    }
}
...
void SPU::render(int num_samples, std::vector<float>& dl, std::vector<float>& dr, std::vector<float>& wl, std::vector<float>& wr, float samples_per_tick) {
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        for (auto& v : active_voices) {
            ...
            if (v.vibrato_enabled && v.vibrato_attack_counter < v.vibrato_attack_time) {
                v.tick_accumulator += 1.0;
                while (v.tick_accumulator >= samples_per_tick) {
                    v.tick_accumulator -= samples_per_tick;
                    v.vibrato_attack_counter += v.vibrato_attack_speed;
                    if (v.vibrato_attack_counter > v.vibrato_attack_time) break;
                }
            }
            ...
            if (v.vibrato_enabled) {
                ...
                float attack = (float)v.vibrato_attack_counter / (float)v.vibrato_attack_time;
                if (attack > 1.0f) attack = 1.0f;
                vibrato_pitch_offset *= attack;
            }
...
bool SQParser::load(const std::string& filename) {
    ...
    for(int i=0; i<16; i++) {
        ...
        SQChannelInit init;
        init.prog_idx = data[ch_off + 2];
        init.vol = data[ch_off + 3];
        init.pan = data[ch_off + 4];
        init.modulation = data[ch_off + 9];
        init.pitch_bend = data[ch_off + 10];
        init.vibrato = data[ch_off + 12];
...
void SQParser::parse_events() {
    ...
    while (cursor < data.size()) {
        ...
        if (cmd == 0x80 || cmd == 0x90) {
            int note = data[cursor++]; int vel = data[cursor++];
            events.push_back({delta, "note", cmd, ch, note, vel});
        }
        else if (cmd == 0xB0) {
            int cc = data[cursor++]; int val = data[cursor++];
            events.push_back({delta, "cc", cmd, ch, 0, 0, cc, val});
        }
        else if (cmd == 0xC0) {
            int val = data[cursor++];
            events.push_back({delta, "prog", cmd, ch, 0, 0, val});
        }
        else if (cmd == 0xE0) {
            int val = data[cursor++];
            events.push_back({delta, "pitch", cmd, ch, 0, 0, val});
        }
        else if (cmd == 0xF0) {
            if (status == 0xFF) {
                ...
                if (meta == 0x2F) { events.push_back({delta, "loop_end", 0,0,0,0}); break; }
                    ...
                    if (len == 3) {
                        ...
                        cursor+=3;
                        events.push_back({delta, "tempo", 0,0,0,0, (int)(60000000.0/mpqn)});
                    } else cursor += len;
...
bool SQParser::saveToMidi(const std::string& filename) {
    ...
    out.write("MThd", 4);
    u32 head = __builtin_bswap32(6); out.write((char*)&head, 4);
    u16 fmt = 0; out.write((char*)&fmt, 2);
    u16 trks = __builtin_bswap16(1); out.write((char*)&trks, 2);
    ...
    u32 len = __builtin_bswap32(track.size());
    out.write((char*)&len, 4);
    out.write((char*)track.data(), track.size());
    return true;
}
...
void MidiParser::parse_midi() {
    events.clear();
    u16 num_trks = Util::readU16BE(data, 10);
    ticks_per_quarter = Util::readU16BE(data, 12);
    if(ticks_per_quarter & 0x8000) ticks_per_quarter = 480; // Smpte ignored

    std::vector<AbsEvent> all;
    size_t cursor = 14;

    for(int t=0; t<num_trks; t++) {
        if(cursor+8 > data.size()) break;
        if(std::memcmp(data.data()+cursor, "MTrk", 4)!=0) break;
        u32 len = Util::readU32BE(data, cursor+4);
        cursor += 8;
        ...
        while(cursor < end && cursor < data.size()) {
            auto res = read_varlen(data, cursor);
            cur_time += res.first; cursor = res.second;
            if(cursor>=data.size()) break;

            u8 st = data[cursor];
            if(st >= 0x80) { cursor++; if(st < 0xF0) running = st; }
            else st = running;

            if(st == 0xFF) {
                u8 type = data[cursor++];
                auto lenRes = read_varlen(data, cursor);
                size_t mlen = lenRes.first; cursor = lenRes.second;
                if(type == 0x51 && mlen==3) {
                    u32 mpqn = (data[cursor]<<16)|(data[cursor+1]<<8)|data[cursor+2];
                    SQEvent e; e.type="tempo"; e.val = (int)(60000000.0/mpqn);
                    all.push_back({cur_time, e});
                }
                else if(type == 0x2F) { /* End track */ }
                cursor += mlen;
            } else if(st == 0xF0 || st == 0xF7) {
                auto l = read_varlen(data, cursor); cursor = l.first + l.second;
            } else {
                ...
                else if(e.cmd == 0xE0) { cursor++; e.type="pitch"; e.val=data[cursor++]; }
                else { cursor++; }
                all.push_back({cur_time, e});
            }
        }
    ...
    u32 prev = 0;
    for(const auto& ae : all) {
        SQEvent e = ae.ev;
        e.delta = ae.abs_time - prev;
        events.push_back(e);
        prev = ae.abs_time;
    }
...
bool ExportSequenceToWav(const std::string& sqPath, const std::string& wavPath, HDParser* hd, BDParser* bd, bool useReverb, bool isMidi) {
    ...
    while (event_idx < seq->events.size()) {
        ...
        if (ev.delta > 0) {
            ...
            if (num_samples > 0) {
                ...
                spu.advance_vibrato_ticks(ev.delta);
    ...
    int tail = 44100 * 2;
    std::vector<float> dl, dr, wl, wr;
    spu.render(tail, dl, dr, wl, wr, 44100.0f);
    if(useReverb) {
        std::vector<float> rl, rr; spu.reverb.process(wl, wr, rl, rr);
        for(size_t i=0; i<dl.size(); i++) { mix_l.push_back(dl[i] + rl[i]*0.5f); mix_r.push_back(dr[i] + rr[i]*0.5f); }
    } else {
        mix_l.insert(mix_l.end(), dl.begin(), dl.end());
        mix_r.insert(mix_r.end(), dr.begin(), dr.end());
    }
    ...
    std::vector<s16> pcm; pcm.reserve(mix_l.size() * 2);
